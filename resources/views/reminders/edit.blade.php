<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Reminder') }} {{-- Updated title for editing --}}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border border-gray-200">
                <div class="p-6 sm:p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('reminders.update', $reminder) }}" class="space-y-6">
                        @csrf
                        @method('PUT') {{-- This is ESSENTIAL for updates --}}

                        {{-- Title Section --}}
                        <div>
                            <x-input-label for="title" :value="__('Reminder Title')" class="font-medium text-gray-700" />
                            <x-text-input id="title" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="text" name="title" :value="old('title', $reminder->title)" required autofocus />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $reminder->description) }}</textarea>
                    ...
                    <x-input-error :messages="$errors->get('notify_minutes_before')" class="mt-2" />
                        ...
                        <div>
                            ...
                            <x-text-input id="guest_emails" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="email" name="guest_emails" :value="old('guest_emails', $reminder->guest_emails)" placeholder="guest1@example.com, guest2@example.com" multiple />
                            <x-input-error :messages="$errors->get('guest_emails')" class="mt-2" />
                            <p class="mt-1 text-xs text-gray-500">Enter guest emails separated by commas. They will receive a notification too.</p>
                        </div>
